...
function policy_q8_H1(gs::game_state, adm_movement)
    ...
    i = 1
        while !find_interesting_mov && i<length(scores)
            find_interesting_mov = (is_opponent_here[i]==n-1)
            i+=1
        end
...
function is_opponent_far_from_winning(gs::game_state, other_player::Int, threshold = 0.07)
    col_closed = 0
...
function policy_q8_H1(gs::game_state)
